...
export class VehicleRenderer {
    ...
    drawVehicleBody(context, centerX, centerY, width, height, color, kind, isPlayer, isPassed) {
        ...
        if (kind === 'truck')
            this.drawTruck(context, centerX, centerY, width, height, color);
        else if (kind === 'bus')
            this.drawBus(context, centerX, centerY, width, height, color);
        else if (kind === 'pickup')
            this.drawPickup(context, centerX, centerY, width, height, color);
        else if (kind === 'motorcycle')
            this.drawMotorcycle(context, centerX, centerY, width, height, color);
        else
            this.drawCar(context, centerX, centerY, width, height, color, isPlayer);
        context.restore();
    }
    drawPlayerMotionTrail(context, centerX, centerY, width, height, speedIntensity, passingPulseSeconds) {
        if (speedIntensity < 0.12 && passingPulseSeconds <= 0)
            return;
