...
export class SpawnSystem {
    ...
    update(deltaSeconds, playerDistanceMeters, playerSpeedKmh, existingTrafficVehicles = []) {
        ...
        if (this.rearTrafficIsEnabled())
            this.rearTrafficTimer -= deltaSeconds;
    ...
    createObstacle(playerDistanceMeters) {
        ...
        return Obstacle.createStaticAccident(this.nextId('obstacle'), x, worldDistanceMeters, firstKind, this.pickVehicleColor(firstKind), secondKind, this.pickVehicleColor(secondKind), this.random);
    }
    createCargo(playerDistanceMeters) {
        const laneIndex = this.random.integer(0, GameConfig.road.laneCount - 1);
        const bonus = this.random.pick([10, 20, 30, 50, 75, 100, 150, 200, 300, 500]);
        return new Cargo(this.nextId('cargo'), laneCenter(laneIndex) + this.random.range(-24, 24), playerDistanceMeters + this.random.range(GameConfig.world.entitySpawnAheadMeters + 20, GameConfig.world.entitySpawnAheadMeters + 120), bonus);
    }
    pickTrafficKind() {
    ...
    pickVehicleColor(kind) {
        if (kind === 'truck')
            return this.random.pick(['#e6e2d5', '#ffb44a', '#9bd3ff', '#d1d5db']);
        if (kind === 'bus')
            return this.random.pick(['#ffce2e', '#69d2ff', '#a5f26c', '#f785a2']);
        if (kind === 'motorcycle')
            return this.random.pick(['#e11d48', '#1d4ed8', '#101827', '#f97316']);
